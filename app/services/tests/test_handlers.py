...
from app.actions.rmwhub import RmwUpdates
...
async def test_handler_action_pull_observations(
    ...
    mocker.patch("app.actions.rmwhub.RmwHubAdapter.download_data", return_value=(
        RmwUpdates(sets=items),
        []
    ))
    mocker.patch("app.actions.rmwhub.RmwHubAdapter.process_updates", return_value=mock_rmw_observations)
