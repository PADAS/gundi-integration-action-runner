...
async def test_hung_dns_resolution_is_rejected_not_awaited_forever(mocker):
    ...
    import gundi_action_runner.services.url_policy as url_policy
    ...
    mocker.patch("gundi_action_runner.services.url_policy.asyncio.get_running_loop", return_value=mock_loop)
...
def test_redact_url_drops_path_userinfo_and_query(url, secret):
    ...
    exists to protect."""
    from gundi_action_runner.services.webhooks import _redact_url
...
async def test_forward_failure_log_does_not_carry_the_url_secret(mocker, caplog):
    ...
    caplog.set_level(logging.WARNING, logger="gundi_action_runner.services.webhooks")
