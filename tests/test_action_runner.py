...
from gundi_action_runner.actions.core import AuthActionConfiguration, ReferenceActionConfiguration
from gundi_action_runner.actions.core import GenericActionConfiguration
from gundi_action_runner.testing.fixtures import AsyncMock, MockSubActionConfiguration, MockPushActionConfiguration, MockPullActionConfiguration, async_return
from app.main import app
from gundi_action_runner.services.action_scheduler import trigger_action
...
from gundi_action_runner.services.utils import find_config_for_action
...
async def test_execute_pull_action_from_pubsub(
    ...
    mocker.patch("gundi_action_runner.services.action_runner._portal", mock_gundi_client_v2)
    mocker.patch("gundi_action_runner.services.action_runner.config_manager", mock_config_manager)
    mocker.patch("gundi_action_runner.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_runner.publish_event", mock_publish_event)
...
async def test_execute_push_action_from_pubsub(
    ...
    mocker.patch("gundi_action_runner.services.action_runner.action_handlers", mock_action_handlers)
    mocker.patch("gundi_action_runner.actions.action_handlers", mock_action_handlers)
    mocker.patch("gundi_action_runner.services.action_runner._portal", mock_gundi_client_v2)
    mocker.patch("gundi_action_runner.services.action_runner.config_manager", mock_config_manager)
    mocker.patch("gundi_action_runner.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_runner.publish_event", mock_publish_event)
...
async def test_scheduled_pull_action_with_invalid_config_is_skipped(
    ...
    mocker.patch("gundi_action_runner.services.action_runner._portal", mock_gundi_client_v2)
    mocker.patch("gundi_action_runner.services.action_runner.config_manager", mock_config_manager)
    mocker.patch("gundi_action_runner.services.action_runner.state_manager", mock_state_manager)
    mocker.patch("gundi_action_runner.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_runner.publish_event", mock_publish_event)
...
async def test_scheduled_pull_action_invalid_config_skip_survives_publish_failure(
    ...
    mocker.patch("gundi_action_runner.services.action_runner.state_manager", mock_state_manager)
    mocker.patch(
        "gundi_action_runner.services.action_runner.log_action_activity",
...
async def test_trigger_subaction(
    ...
    mocker.patch("gundi_action_runner.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_runner.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_scheduler.publish_event", mock_publish_event)
    integration_id = str(integration_v2.id)
    action_id = "pull_observations_by_date"
...
async def test_execute_action_with_handler_error(
    ...
    mocker.patch("gundi_action_runner.services.action_runner.action_handlers", mock_action_handlers_with_request_errors)
    mocker.patch("gundi_action_runner.services.action_runner._portal", mock_gundi_client_v2)
    mocker.patch("gundi_action_runner.services.action_runner.config_manager", mock_config_manager)
    mocker.patch("gundi_action_runner.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_runner.publish_event", mock_publish_event)
...
async def test_execute_action_reports_gundi_api_error_with_response_details(
    ...
    mocker.patch("gundi_action_runner.services.action_runner.action_handlers", {"pull_observations": (handler, MockPullActionConfiguration, None)})
    mocker.patch("gundi_action_runner.services.action_runner._portal", mock_gundi_client_v2)
    mocker.patch("gundi_action_runner.services.action_runner.config_manager", mock_config_manager)
    mocker.patch("gundi_action_runner.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_runner.publish_event", mock_publish_event)
...
async def test_ephemeral_run_builds_synthetic_integration(
    ...
    mocker.patch("gundi_action_runner.services.action_runner.action_handlers", mock_ephemeral_action_handlers)
    mocker.patch("gundi_action_runner.services.action_runner._portal", mock_gundi_client_v2)
    mocker.patch("gundi_action_runner.services.action_runner.config_manager", mock_config_manager)
    mocker.patch("gundi_action_runner.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_runner.publish_event", mock_publish_event)
...
async def test_ephemeral_handler_httpstatuserror_propagates_upstream_status(
    ...
    mocker.patch("gundi_action_runner.services.action_runner.action_handlers", handlers)
    mocker.patch("gundi_action_runner.services.action_runner._portal", mock_gundi_client_v2)
    mocker.patch("gundi_action_runner.services.action_runner.config_manager", mock_config_manager)
    mocker.patch("gundi_action_runner.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_runner.publish_event", mock_publish_event)
...
async def test_ephemeral_auth_handler_write_is_blocked_end_to_end(
    ...
    from gundi_action_runner.services.gundi import send_events_to_gundi, EphemeralWriteBlocked
...
async def test_execute_action_neither_id_nor_state_direct_call_logs_but_does_not_publish(
    ...
    mocker.patch("gundi_action_runner.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("gundi_action_runner.services.action_runner.publish_event", mock_publish_event)
    caplog.set_level(logging.ERROR, logger="gundi_action_runner.services.action_runner")
...
async def test_ephemeral_configuration_error_forwards_its_message_as_422(
    ...
    from gundi_action_runner.services.errors import IntegrationConfigurationError
...
def _mock_draft_url_resolution(mocker, ip):
    ...
    mocker.patch("gundi_action_runner.services.url_policy._resolve_addresses", AsyncMock(return_value=[ip]))
...
async def test_trigger_action_is_blocked_on_ephemeral_run(mocker, mock_publish_event):
    ...
    from gundi_action_runner.services.activity_logger import ephemeral_run
    from gundi_action_runner.services.gundi import EphemeralWriteBlocked
    mocker.patch("gundi_action_runner.services.action_scheduler.publish_event", mock_publish_event)
...
def test_validation_error_log_does_not_carry_the_request_body(caplog):
    ...
    # may be logged.
    import logging
    caplog.set_level(logging.DEBUG, logger="gundi_action_runner.app_factory")
    body = {
        "action_id": "auth",
        "integration_state": {
            "type_value": "earth_ranger",
...
async def test_ephemeral_handler_error_is_not_logged_verbatim(
    ...
    _patch_ephemeral_runner(mocker, mock_ephemeral_action_handlers, mock_gundi_client_v2, mock_config_manager, mock_publish_event)
    caplog.set_level(logging.DEBUG, logger="gundi_action_runner.services.action_runner")
...
async def test_ephemeral_untrusted_status_code_on_connector_exception(
    ...
    from gundi_action_runner.services.errors import IntegrationAuthError, IntegrationBadResponseError
    import logging
