...
from gundi_action_runner.settings import default_token_cache_url, validated_token_cache_url
...
_REPO_ROOT = pathlib.Path(__file__).resolve().parents[1]  # tests/ sits directly under the repo root here
_SUBPROCESS_KEYS = (
    "GUNDI_TOKEN_CACHE_URL", "REDIS_HOST", "REDIS_PORT", "REDIS_TOKEN_CACHE_DB",
    # create_app() imports these at startup; an exported value (or one leaked
    # by another test) would make the probe import a connector that is not there.
    "GUNDI_HANDLERS_MODULES", "GUNDI_LEGACY_ACTIONS_MODULE", "GUNDI_LEGACY_WEBHOOKS_MODULE",
)
...
@pytest.mark.parametrize("entry_point", [
    "app.main", "app.register", "gundi_action_runner.services.action_runner", "gundi_action_runner.services.config_manager", "gundi_action_runner.services.webhooks",
    ...
    "gundi_action_runner.services.errors", "gundi_action_runner.services.retry_policies", "gundi_action_runner.services.gundi",
])
def test_runner_settings_load_before_the_client_library(entry_point):
    ...
    read_env(), and the first loader wins per key. gundi_action_runner/settings.py imports
    the client only after its own read_env(), so the runner's .env wins wherever
    gundi_action_runner.settings is reached first. Observed by spying on read_env itself:
    ...
    the connector's own settings module, and counting would fail it for doing so."""
...
def test_settings_derive_the_url_from_redis_settings_when_not_overridden():
    line = _settings_in_subprocess(
        ...
        "from gundi_action_runner import settings; print(settings.REDIS_TOKEN_CACHE_DB, settings.GUNDI_TOKEN_CACHE_URL)",
    )
...
def test_settings_honour_the_env_override():
    line = _settings_in_subprocess(
        ...
        "from gundi_action_runner import settings; print(settings.GUNDI_TOKEN_CACHE_URL)",
    )
...
def test_validated_token_cache_url_falls_back_to_memory_with_one_warning(bad, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="gundi_action_runner.settings"):
...
def test_bare_client_reads_the_installed_url_at_construction_time(monkeypatch):
    ...
    GundiClient() in the process from gundi_action_runner.settings)."""
...
def test_settings_load_before_connector_code_runs():
    """gundi_action_runner.actions executes the connector's handlers module at import. A
    GundiClient() built at module scope there must already see the runner's
    token cache URL, so importing gundi_action_runner.actions alone has to load gundi_action_runner.settings
    ...
    probe = (
        "import sys; import gundi_action_runner.actions; "
        "assert 'gundi_action_runner.settings' in sys.modules, 'gundi_action_runner.settings not loaded by gundi_action_runner.actions'; "
        "from gundi_action_runner import settings; from gundi_client_v2 import settings as cs; "
...
async def test_a_portal_token_the_api_rejects_is_replaced_by_the_client(mocker):
    ...
    """
    from gundi_action_runner.services import gundi as gundi_helpers
